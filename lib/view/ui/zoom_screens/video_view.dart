  ...
  Widget build(BuildContext context) {
    ...
    useEffect(() {
      ...
      updateAudioStatus() async {
        ...
        if (talking) {
          Timer(const Duration(milliseconds: showTalkingIconDuration), () {
            if (isMounted()) {
              isTalking.value = false;
            }
          });
    ...
    if (fullScreen) {
      log("message is fullscreen");
      if (sharing || isVideoOn.value || isPiPView || preview) {
        return Container(
          height: MediaQuery.of(context).size.height,
          width: MediaQuery.of(context).size.width,
          alignment: Alignment.center,
          child: FlutterZoomView.View(
            key: Key(viewParamStr),
...
class VideoView2 extends FlutterZoomView.ZoomView {
  const VideoView2({
    super.key,
    required super.user,
    required super.sharing,
  ...
  Widget build(BuildContext context) {
    ...
    var isMuted = useState(false);
    var isMounted = useIsMounted();
    var zoom = getIt<OnlineConsultManager>().zoom;
    useState(false);
    // zoom.user?.audioStatus?.isMuted().then((muted) => isMuted.value = muted);
